...
class SavingCrud:
    def create_saving_table(db: AsyncSession, data: PSavingHistory) -> bool:
        try:
            # sub → uuid
    ...
    def select_saving_table(db: AsyncSession, sub: str):
        try:
            items = (
                db.query(
    ...
    def select_saving_table_amount(db: AsyncSession, sub: str):
        try:
            result = db.execute(
